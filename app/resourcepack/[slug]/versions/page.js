...
export async function generateMetadata({ params }) {
  try {
    ...
    return { title: `${pack.title} - Версии | White Minecraft` }
  } catch {
              ...
              <div className="flex flex-wrap items-center gap-3 md:gap-4 text-xs md:text-sm">
                ...
                <div className="hidden sm:flex flex-wrap gap-1.5">
                  {pack.categories.slice(0, 4).map((cat) => (
                    <Link key={cat} href={`/resourcepacks?f=categories:${cat}`} className="px-2 py-0.5 text-xs font-semibold rounded-full hover:underline transition-all" style={{ backgroundColor: '#34363c', color: '#80878f' }}>{cat}</Link>
                  ))}
                </div>
              </div>
            </div>
