  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-4 md:mb-6 flex items-center gap-2 text-sm flex-wrap">
        <Link href="/shaders" className="text-gray-400 hover:text-modrinth-green transition-colors duration-200 font-medium">Шейдеры</Link>
      ...
      <div className="border-b pb-4 md:pb-6 mb-6 md:mb-8" style={{ borderBottomColor: '#34363c' }}>
        <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 lg:items-start">
          <div className="flex gap-3 md:gap-4 flex-1">
            {shader.icon_url && (<img src={shader.icon_url} alt={shader.title} className="w-16 h-16 md:w-20 md:h-20 rounded-lg object-cover flex-shrink-0" />)}
              ...
              <div className="hidden lg:flex flex-wrap items-center gap-3 md:gap-4 text-xs md:text-sm">
                ...
                <div className="hidden sm:flex flex-wrap gap-1.5">
                  {shader.categories.slice(0, 4).map((cat) => (
                    <Link key={cat} href={`/shaders?f=categories:${cat}`} className="px-2 py-0.5 text-xs font-semibold rounded-full hover:underline transition-all" style={{ backgroundColor: '#34363c', color: '#80878f' }}>{cat}</Link>
                  ))}
                </div>
              </div>
            </div>
